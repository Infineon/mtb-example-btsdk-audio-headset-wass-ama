# Headset WASS AMA app

## Overview
This sample application is used to demonstrate Wireless Audio Stereo Sync (WASS) with AMA.

## Instructions
This demo requires two boards. One will act as the Primary device (connected to the phone) while
the second will act as the Secondary device (eavesdropping device).

To compile and download the application, use the following command (TARGET value depends on which boards you are using, specify the HCI UART port that each board is connected to, such as COM1 on Windows):
$ make program TARGET=CYW920721M2EVK-02  UART=xxx

Once the app has been downloaded to both boards, you MUST run the audio-lib-pro/utils/lrac\_config.exe application
to configure the boards.
To compile the lrac\_config.exe app (Cygwin):<br/>
$cd audio-lib-pro/utils/lrac\_headset/lrac\_config<br/>
$make

Use the config.sh script (which executes lrac\_config.exe) to configure two boards.
This script takes three arguments - the HCI UART ports of the two devices, and an optional value between 10-98 which will be used for the last octet of the BdAddr seed xx below.  If the third parameter is omitted a random value is generated.

This script:

- Creates two consecutive BdAddr (20-71-9B-10-00-xx)
- Configure the First board (first UART port on the command line) as:
     - Primary
     - Left Side
     - Local BdAddr: 20-71-9B-10-00-xx
     - Peer BdAddr: 20-71-9B-10-00-xx+1
- Configure the Second board (second UART port on the command line) as:
     - Secondary
     - Right Side
     - Local BdAddr: 20-71-9B-10-00-xx+1
     - Peer BdAddr: 20-71-9B-10-00-xx

Example:<br/>
$./config.sh COM14 COM16 20

Once the two boards are configured (the configuration is saved in NVRAM), you can use a third
device (e.g. iPhone or Android) to Search and Connect to the Primary device.
The name of the device will be:

  'WASS 20' (if 20 was used as the third argument for example)

The connection between the two boards will be automatically established, either after the board is started or once the phone connects.
Note that to start the boards, the 'Action' button must be pressed (the Custom button on an EVK board).

The Action button is a multi-purpose button. It can be used to:

 - Start the board
 - Request a connection to the phone
 - Play/Pause an A2DP stream
 - Accept/Reject/HangUp HFP calls

## Button Functions
- On CYW920721M2EVK-01/EVK-02/EVB-03(3 buttons)<br/>
Button event: click/ long press/ hold<br/>
1. Custom: Play, Pause, Accept/ Reject, Discoverable/ -<br/>
2. Vol+: Volume up/ Next Track/ Voice recognition<br/>
...
- AAC\_SUPPORT
    - This option allows the device to enable the AAC codec if the Bluetooth&#174; chip supports. By default headset_wass_ama doesn't support it.
