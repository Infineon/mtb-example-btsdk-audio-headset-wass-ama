...
# pro-only prebuilt libs
CY_IGNORE+=audio-lib-pro/utils
CY_APP_PATCH_LIBS_PRO_PATH=audio-lib-pro/prebuilt
CY_APP_PATCH_LIBS_PRO += average_rssi_lib.a
CY_APP_PATCH_LIBS_PRO += lr_cync_lib.a
...
ifneq ($(PLATFORM_DEBUG), 0)
CY_APP_PATCH_LIBS += patch_debug.a
endif
CY_APP_PATCH_LIBS += wiced_app_watchdog_lib.a
...
CY_APP_DEFINES += -DAPP_BLE_DEVICE_NAME='"HeadsetWassAma"'
CY_APP_DEFINES += -DAMA_AUDIO_SNK_INCLUDED
CY_APP_DEFINES += -DAMA_HANDSFREE_INCLUDED
endif
#CY_APP_DEFINES += -DAPP_OFU_DEBUG
CY_APP_DEFINES += -DAPP_OFU_SUPPORT
...
CY_APP_DEFINES += -DBT_HS_SPK_CONTROL_LINK_KEY_COUNT=6
#CY_APP_DEFINES += -DHCI_TRACE_OVER_TRANSPORT
CY_APP_DEFINES += -DLRAC_TRACE_DBG_ENABLED=0
ifneq ($(PLATFORM_DEBUG), 0)
CY_APP_DEFINES += -DPLATFORM_DEBUG
endif
CY_APP_DEFINES += -DPRE_INIT_LIB
CY_APP_DEFINES += -DWICED_BT_A2DP_SINK_DEBUG=0
CY_APP_DEFINES += -DWICED_APP_LE_INCLUDED=TRUE
#CY_APP_DEFINES += -DWICED_BT_HFP_HF_WBS_INCLUDED=0
...
CY_APP_DEFINES += -DVOLUME_EFFECT
CY_APP_DEFINES += -DDISABLE_SNIFF_MODE_DURING_A2DP
...
# For HEADSET_WASS_AAC, the replaced packet will add extra 4 bytes
# Redefine MTU as 668 to prevent the packet split for 2DH5 packets
CY_APP_DEFINES += -DWICED_BT_A2DP_SINK_AVDT_SIG_CH_MTU=668
else
...
ifeq ($(APP_TRACE), 1)
CY_APP_DEFINES += -DAPP_TRACE_ENABLED
endif
...
ifeq ($(VOICE_PROMPT), 1)
COMPONENTS += voice_prompt_lib
COMPONENTS += vpc_decoder_lib
CY_APP_DEFINES += -DVOICE_PROMPT
# Remove LRAC Debug messages to save RAM
CY_APP_DEFINES += -DLRAC_TRACE_ERR_ENABLED=0
endif
