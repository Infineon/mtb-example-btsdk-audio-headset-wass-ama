 ...
 * Definitions
 */
/* Callback function to send OFU Events (Started/Completed/Aborted) to application */
typedef void (app_ofu_clt_app_callback_t)(app_ofu_event_t event);

/* Callback function to send OFU data to the peer (Server) device */
typedef wiced_result_t (app_ofu_clt_send_callback_t)(uint8_t header, uint8_t *p_data,
        uint16_t length);
